\documentclass[letterpaper,11pt]{article}

\usepackage{latexsym}
\usepackage[empty]{fullpage}
\usepackage{titlesec}
\usepackage{marvosym}
\usepackage[usenames,dvipsnames]{color}
\usepackage{verbatim}
\usepackage{enumitem}
\usepackage[hidelinks]{hyperref}
\usepackage{fancyhdr}
\usepackage[english]{babel}
\usepackage{tabularx}
\usepackage{awesomebox}
\usepackage{fontawesome5}

\usepackage{comment}

\input{glyphtounicode}

\pagestyle{fancy}
\fancyhf{} % clear all header and footer fields
\fancyfoot{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}

% Adjust margins
\addtolength{\oddsidemargin}{-0.5in}
\addtolength{\evensidemargin}{-0.5in}
\addtolength{\textwidth}{1in}
\addtolength{\topmargin}{-.5in}
\addtolength{\textheight}{1.0in}

\urlstyle{same}

\raggedbottom
\raggedright
\setlength{\tabcolsep}{0in}

% Sections formatting
\titleformat{\section}{
  \vspace{-4pt}\scshape\raggedright\large
}{}{0em}{}[\color{black}\titlerule \vspace{-5pt}]

% Ensure that generate pdf is machine readable/ATS parsable
\pdfgentounicode=1

%-------------------------
% Custom commands
\newcommand{\resumeItem}[2]{
  \item\small{
    \textbf{#1}{: #2 \vspace{-2pt}}
  }
}
\newcommand{\resumeItemSimple}[1]{
  \item\small{
    {#1}\vspace{-2pt}
  }
}
\newcommand{\resumeItemSimpleNoBullet}[1]{
  \vspace{-5pt}\item[]\small{
    {#1}\vspace{-2pt}
  }
}

\newcommand{\Paper}[3]{
  \vspace{-1pt}\item \small
    \hangindent=1.4em \hangafter=1
    \textbf{#1}. #2
    \par{\footnotesize #3}
  \vspace{-4pt}
}

\newcommand{\PaperTLDR}[4]{
  \vspace{-1pt}\item \small
    \hangindent=1.4em \hangafter=1
    \textbf{#1}. #2
    \par{\footnotesize #3}
    \if\relax\detokenize{#4}\relax\else
      \par{\scriptsize \emph{TLDR: #4}}
    \fi
  \vspace{-4pt}
}

\newcommand{\Thesis}[2]{
  \vspace{-1pt}\item
    \begin{tabular*}{0.97\textwidth}[t]{l@{\extracolsep{\fill}}r}
      \small\textbf{#1} \\
      {\small #2} \\
    \end{tabular*}\vspace{-5pt}
}

\newcommand{\ExperienceListStart}{
    \vspace{-1pt}\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=0pt,parsep=0pt]
}

\newcommand{\ExperienceListEnd}{
    \end{itemize}\vspace{-5pt}
}

\newcommand{\Experience}[3]{
    \item \small \textbf{#1} \textit{(#2)}: #3
}

\newcommand{\AwardListStart}{
    \vspace{-1pt}\begin{tabular*}{0.97\textwidth}{l@{\extracolsep{\fill}}r}
}

\newcommand{\AwardListEnd}{
    \end{tabular*}\vspace{-5pt}
}

\newcommand{\Award}[2]{
    $\bullet$ {\small#1} & \textit{\small #2} \\
}

% Just in case someone needs a heading that does not need to be in a list
\newcommand{\resumeHeading}[4]{
    \begin{tabular*}{0.99\textwidth}[t]{l@{\extracolsep{\fill}}r}
      \textbf{#1} & #2 \\
      \textit{\small#3} & \textit{\small #4} \\
    \end{tabular*}\vspace{-5pt}
}

\newcommand{\resumeSubheading}[4]{
  \vspace{-1pt}\item
    \begin{tabular*}{0.97\textwidth}[t]{l@{\extracolsep{\fill}}r}
      \small\textbf{#1} & \small#2 \\
      \textit{\small#3} & \textit{\small #4} \\
    \end{tabular*}\vspace{-5pt}
}

\newcommand{\resumeSubheadingSimple}[2]{
  \vspace{-1pt}\item
    \begin{tabular*}{0.97\textwidth}[t]{l@{\extracolsep{\fill}}r}
      \small\textbf{#1} & \small\textit{\small #2} \\
    \end{tabular*}\vspace{-7pt}
}

\newcommand{\EducationEntry}[2]{
  \vspace{-1pt}\item
    \begin{tabular*}{0.97\textwidth}[t]{@{}>{\raggedright\arraybackslash}p{0.78\textwidth}@{\extracolsep{\fill}}r@{}}
      \small #1 & \small\textit{#2} \\
    \end{tabular*}\vspace{-7pt}
}

\newcommand{\resumeSubSubheading}[2]{
    \begin{tabular*}{0.97\textwidth}{l@{\extracolsep{\fill}}r}
      \textit{\small#1} & \textit{\small #2} \\
    \end{tabular*}\vspace{-5pt}
}

\newcommand{\resumeSubItem}[2]{\resumeItem{#1}{#2}\vspace{-4pt}}

\newcommand{\resumeSubItemSimple}[1]{\resumeItemsimple{#1}\vspace{-4pt}}

\renewcommand{\labelitemii}{$\circ$}

\newcommand{\resumeSubHeadingListStart}{\begin{itemize}[leftmargin=*]}
\newcommand{\resumeSubHeadingListEnd}{\end{itemize}}
\newcommand{\educationListStart}{\begin{itemize}[leftmargin=*, itemsep=0pt, topsep=0pt, parsep=0pt]}
\newcommand{\educationListEnd}{\end{itemize}}
\newcommand{\compactListStart}{\begin{itemize}[leftmargin=*, itemsep=0pt, topsep=0pt, parsep=0pt]}
\newcommand{\compactListEnd}{\end{itemize}}
\newcommand{\resumeItemListStart}{\begin{itemize}}
\newcommand{\resumeItemListEnd}{\end{itemize}\vspace{-5pt}}

%-------------------------------------------
%%%%%%  CV STARTS HERE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%


\begin{document}

%----------HEADING-----------------
\begin{center}
{\LARGE \scshape \bfseries Yan Ma} \\ \vspace{2pt}
{\footnotesize 860+ citations, 1.3k+ GitHub stars across research projects} \\ \vspace{2pt}
(+86) 152-6982-9096 $\bullet$ 
\href{mailto:yanma23@m.fudan.edu.cn}{yanma23@m.fudan.edu.cn} $\bullet$
\href{https://yanmaaaa.github.io}{Homepage} $\bullet$
\href{https://scholar.google.com/citations?user=tnJyAAoAAAAJ\&hl=en}{Google Scholar} $\bullet$
\href{https://github.com/mantle2048}{Github} $\bullet$
\href{https://yanmaaaa.github.io/assets/research_statement.pdf}{Research Statement}
\end{center}


%-----------Projects-----------------
\section{Research Interests}
    My research aims to build multimodal agents for partially observed environments, especially how they perceive, reason, act, and predict over long-horizon tasks. So far, my work has mainly studied how RL changes multimodal models, including training dynamics, the joint improvement of reasoning and perception, and agentic vision tool use. Recently, I am interested in predictive multimodal learning from video, especially how video data can be cleaned, filtered, and annotated to better support the anticipation of future observations and action outcomes.


%-----------EDUCATION-----------------
\section{Education}
  \educationListStart
    \EducationEntry{\textbf{Ph.D. Student, Fudan University} (Advisors: Prof. \href{https://scholar.google.co.jp/citations?user=oIz_CYEAAAAJ\&hl=en}{Pengfei Liu} and Prof. \href{https://mmlab.siat.ac.cn/yuqiao}{Yu Qiao})}{2023.09 -- 2027.06}
    \EducationEntry{\textbf{Master in Computer Application Technology, Fudan University}}{2020.09 -- 2023.06}
    \EducationEntry{\textbf{Bachelor in Computer Science, Dalian University of Technology} (GPA: 4.076/5)}{2016.09 -- 2020.06}
  \educationListEnd

%-----------Experience-----------------
\section{Experience}
  \ExperienceListStart
    % \Experience{Shanghai Innovation Institute}{2026.03 -- 2026.06}{leading a large-scale audio-video data processing pipeline}
    \Experience{MiniMax \& HailuoAI}{2025.01 -- 2026.02}{worked on multimodal foundation models for visual reasoning and vision tool use in the M-series, and image caption systems for Hailuo video generation}
    \Experience{Shanghai AI Laboratory}{2023.07 -- 2025.01}{worked on text generation and unified multimodal model training}
  \ExperienceListEnd

%-----------Publications-----------------
\section{Publications}
  \resumeSubHeadingListStart
  \PaperTLDR
  {VidaForge: Building a Video Foundation Model Pretraining Data Pipeline from Scratch in an Academic Lab \href{https://yanmaaaaaa.notion.site/vidaforge}{[\faGlobe]}\href{https://github.com/GAIR-NLP/VidaForge}{[\faGithub]}}
  {\textit{Technical Blog}, July 4, 2026}
  {\underline{Yan Ma}, Jiadi Su, Zhulin Hu, Ethan Chern, Linhao Zhang, TianTian Mi, Pengfei Liu}
  {We document a practical pipeline for building video foundation model pretraining data from scratch.}
  \PaperTLDR
  {What Does Vision Tool-Use Reinforcement Learning Really Learn? Disentangling Tool-Induced and Intrinsic Effects for Crop-and-Zoom \href{https://arxiv.org/abs/2602.01334}{[\faFilePdf]}\href{https://github.com/GAIR-NLP/Med}{[\faGithub]}}
  {\textit{ICML 2026}}
  {\underline{Yan Ma}$^\dagger$, Weiyu Zhang$^\dagger$, Tianle Li, Linge Du, Xuyang Shen, Pengfei Liu}
  {We show current vision tool-use RL gains are driven mainly by intrinsic capability improvement and reduced tool-induced harm, not true tool-based correction.}
  \PaperTLDR
  {One RL to See Them All: Visual Triple Unified Reinforcement Learning \href{https://arxiv.org/abs/2505.18129}{[\faFilePdf]}\href{https://github.com/MiniMax-AI/One-RL-to-See-Them-All}{[\faGithub]}}
  {\textit{arXiv:2505.18129}\tiny[COLM 2026 under review]}
  {\underline{Yan Ma}$^\dagger$, Linge Du$^\dagger$, Xuyang Shen$^\dagger$, Shaoxiang Chen, Pengfei Li, Qibing Ren, Lizhuang Ma, Yuchao Dai, Pengfei Liu, Junjie Yan}
  {We build a unified RL system (V-Triune) for jointly training visual reasoning and perception in VLMs, yielding broad gains across both task types.}
  \PaperTLDR
  {Rethinking RL Scaling for Vision Language Models: A Transparent, From-Scratch Framework and Comprehensive Evaluation Scheme \href{https://arxiv.org/abs/2504.02587}{[\faFilePdf]}\href{https://github.com/GAIR-NLP/MAYE}{[\faGithub]}}
  {\textit{arXiv:2504.02587}}
  {\underline{Yan Ma}, Steffi Chern, Xuyang Shen, Yiran Zhong, Pengfei Liu}
  {We introduce a transparent, from-scratch RL framework and trajectory-level evaluation for VLMs, showing stronger generalization than SFT and clearer training-dynamics insights.}
  \PaperTLDR
  {ANOLE: An Open, Autoregressive, Native Large Multimodal Models for Interleaved Image-Text Generation \href{https://arxiv.org/abs/2407.06135}{[\faFilePdf]}\href{https://iclr-blogposts.github.io/2025/blog/fine-tuning-token-based-large-multimodal-models/}{[\faGlobe]}\href{https://github.com/GAIR-NLP/Anole}{[\faGithub]}}
  {\textit{arXiv:2407.06135; extended version accepted to ICLR 2025 Blog Track}}
  {Ethan Chern*, Jiadi Su*, \underline{Yan Ma*}, Pengfei Liu}
  {We present an open native autoregressive multimodal model for coherent interleaved image-text generation, with efficient fine-tuning from Chameleon.}
  {}
  \PaperTLDR
  {MoPS: Modular Story Premise Synthesis for Open-Ended Automatic Story Generation \href{https://arxiv.org/abs/2406.05690}{[\faFilePdf]}\href{https://github.com/GAIR-NLP/MoPS}{[\faGithub]}}
  {\textit{ACL 2024}}
  {\underline{Yan Ma}, Yu Qiao, Pengfei Liu}
  {}
  % {We modularize story-premise synthesis into structured design and integration steps, producing more diverse and higher-quality premises for downstream story generation.}
  % \Paper
  % {Weak-to-Strong Reasoning \small \href{https://arxiv.org/abs/2407.13647}{[\faFilePdf]}\href{https://github.com/GAIR-NLP/weak-to-strong-reasoning}{[\faGithub]}}
  % {\textit{EMNLP Findings 2024}}
  % {Yuqing Yang, \underline{Yan Ma}, Pengfei Liu}
  % \Paper
  % {OlympicArena: Benchmarking Multi-discipline Cognitive Reasoning for Superintelligent AI \href{https://arxiv.org/abs/2406.12753}{[\faFilePdf]}\href{https://github.com/GAIR-NLP/OlympicArena}{[\faGithub]}}
  % {\textit{NeurIPS Dataset \& Benchmark Track 2024}}
  % {Zhen Huang, Zengzhi Wang, Shijie Xia, ..., \underline{Yan Ma}, ..., Shaoting Zhang, Dahua Lin, Yu Qiao, Pengfei Liu}
  \resumeSubHeadingListEnd

\section{Technical Skills \& Academic Service}
    \compactListStart
    \item{
        \small Verl, Transformers, Ray, FSDP, vLLM, SGLang, \href{https://github.com/mantle2048/drq_jax}{Jax}, FFmpeg, Codex, Git, NeoVim, Tmux
    }
    \item{ \small Reviewer for AAAI 2025, NeurIPS 2025, COLM 2025--2026, ICLR 2026, ICML 2026, and ACL ARR 2026}
  \compactListEnd
%-------------------------------------------
\end{document}
